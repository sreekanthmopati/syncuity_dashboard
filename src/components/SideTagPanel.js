...
const SideTagPanel = () => {
  ...
  // Static data for petrol bunk and kalyanamandapam assets with financial changes
  useEffect(() => {
    const loadUpdatedAssets = () => {
      try {
        const staticData = [
          {
            id: 1,
            asset_name: "City Petrol Bunk",
            last_updated: new Date().toISOString(),
            changes: [
              { field: "direct_income", old_value: 125000, new_value: 142000 },
              { field: "fuel_inventory", old_value: 15000, new_value: 11200 },
              { field: "daily_spends", old_value: 45000, new_value: 52000 }
            ]
          },
          {
            id: 2,
            asset_name: "Highway Fuel Station",
            last_updated: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(),
            changes: [
              { field: "direct_income", old_value: 185000, new_value: 210000 },
              { field: "lubricant_sales", old_value: 25000, new_value: 32000 },
              { field: "maintenance_spends", old_value: 15000, new_value: 18000 }
            ]
          },
          {
            id: 3,
            asset_name: "Grand Kalyanamandapam",
            last_updated: new Date(Date.now() - 1 * 60 * 60 * 1000).toISOString(),
            changes: [
              { field: "booking_income", old_value: 75000, new_value: 95000 },
              { field: "catering_income", old_value: 45000, new_value: 52000 },
              { field: "decorations_spend", old_value: 25000, new_value: 32000 }
            ]
          },
          {
            id: 4,
            asset_name: "Premium Marriage Hall",
            last_updated: new Date(Date.now() - 4 * 60 * 60 * 1000).toISOString(),
            changes: [
              { field: "hall_rental", old_value: 120000, new_value: 135000 },
              { field: "ac_rental", old_value: 15000, new_value: 18000 },
              { field: "staff_expenses", old_value: 35000, new_value: 42000 }
            ]
          },
          {
            id: 5,
            asset_name: "CNG Fuel Station",
            last_updated: new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString(),
            changes: [
              { field: "cng_sales", old_value: 85000, new_value: 102000 },
              { field: "cng_inventory", old_value: 8000, new_value: 6500 },
              { field: "operational_cost", old_value: 28000, new_value: 35000 }
            ]
          }
        ];
        
        setUpdatedAssets(staticData);
        console.log('Updated assets loaded:', staticData);
      } catch (err) {
        console.error(err);
        setError('Error loading updated assets.');
      } finally {
        setIsLoading(false);
      }
    };
    ...
    if (isOpen) {
      ...
      setTimeout(loadUpdatedAssets, 800);
    }
  }, [isOpen]);
  ...
  const getPanelStyle = () => {
    const isMobile = windowSize.width < 640;
    const maxWidth = isMobile ? windowSize.width * 0.95 : 380;
    const maxHeight = windowSize.height * 0.7;
  ...
  const formatValue = (value, field) => {
    if (typeof value === 'number') {
      if (field.includes('rate') || field.includes('percentage')) {
        return `${value}%`;
      }
      // Format currency values
      if (field.includes('income') || field.includes('sales') || field.includes('spend') || field.includes('expense') || field.includes('rental') || field.includes('cost')) {
                    ...
                    <div className="flex items-center gap-2 relative">
                      ...
                      >
                        <motion.span 
                          className="font-black text-white drop-shadow-lg relative z-10"
                          style={{
                            fontSize: windowSize.width > 1600 ? '1.2rem' : '0.9em',
                            textShadow: '0 0 10px rgba(255, 255, 255, 0.5)'
                          }}
                          whileHover={{ 
                            scale: 1.1,
          ...
          >
            ...
            <div className="px-3 py-2 sm:px-4 sm:py-2.5 border-b border-pink-100/50 bg-gradient-to-r from-pink-600 to-rose-500 flex-shrink-0">
              <div className="flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <h3 className="text-base sm:text-lg font-medium text-white tracking-tight">Asset updates</h3>
                </div>
                <button 
                  onClick={() => setIsOpen(false)}
                  className="text-white/80 hover:text-white transition-colors"
              ...
              <p className="text-pink-100/80 text-xs mt-0.5 ml-8 sm:ml-10">Recent financial updates across assets</p>
            </div>
            ...
            >
              <div className="space-y-4 sm:space-y-5">
                <div>
                  <h4 className="font-medium sm:font-semibold text-base sm:text-lg text-gray-800 mb-2 sm:mb-3 flex items-center">
                    <svg className="w-4 h-4 sm:w-5 sm:h-5 mr-1 sm:mr-2 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                  ...
                  {isLoading && (
                    <div className="flex flex-col items-center justify-center py-4 sm:py-6 space-y-2 sm:space-y-3">
                      <div className="animate-spin rounded-full h-8 w-8 sm:h-10 sm:w-10 border-t-2 border-b-2 border-pink-500"></div>
                      <p className="text-gray-500 text-sm">Loading financial updates...</p>
                    </div>
                  )}
                  ...
                  {!isLoading && !error && updatedAssets.length === 0 && (
                    <div className="p-2 sm:p-4 bg-blue-50 text-blue-700 rounded-lg border border-blue-100 flex items-center">
                      <svg className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      <span className="text-sm">No recent financial updates</span>
                    </div>
                  )}

                  <ul className="mt-2 sm:mt-3 space-y-3 sm:space-y-4">
                    {updatedAssets.map((asset, idx) => (
                      <motion.li
                        key={asset.id}
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: idx * 0.05 }}
                        className="p-3 sm:p-4 bg-white rounded-lg border border-gray-200 hover:shadow-md transition-all"
                      >
                        {/* Asset Header */}
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-start">
                            <span className="flex-shrink-0 bg-green-100 text-green-600 rounded-full p-1.5 sm:p-2 mr-2 sm:mr-3 mt-0.5">
                              <svg className="w-3 h-3 sm:w-4 sm:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                              </svg>
                            </span>
                            <div>
                              <p className="text-sm font-semibold text-gray-800">{asset.asset_name}</p>
                              <div className="flex items-center text-xs text-gray-500 mt-1">
                                <svg className="w-3 h-3 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <span>Updated: {new Date(asset.last_updated).toLocaleTimeString()}</span>
                              </div>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="ml-8 sm:ml-10 space-y-2">
                          {asset.changes.map((change, changeIdx) => {
                            const percentageChange = calculateChange(change.old_value, change.new_value);
                            const isPositive = change.new_value > change.old_value;
                            
                            return (
                              <div key={changeIdx} className="flex items-center justify-between text-sm">
                                <span className="text-gray-600 font-medium">
                                  {formatFieldName(change.field)}:
                                </span>
                                <div className="flex items-center space-x-2">
                                  <span className="text-red-500 line-through bg-red-50 px-1.5 py-0.5 rounded text-xs">
                                    {formatValue(change.old_value, change.field)}
                                  </span>
                                  <svg className="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14 5l7 7m0 0l-7 7m7-7H3" />
                                  </svg>
                                  <div className="flex items-center space-x-1">
                                    <span className={`${isPositive ? 'text-green-600' : 'text-red-600'} bg-${isPositive ? 'green' : 'red'}-50 px-1.5 py-0.5 rounded text-xs font-semibold`}>
                                      {formatValue(change.new_value, change.field)}
                                    </span>
                                    <span className={`text-xs ${isPositive ? 'text-green-600' : 'text-red-600'} font-medium`}>
